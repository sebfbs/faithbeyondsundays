...
-- Fix both community pulse functions to use auth.uid() for caller verification

CREATE OR REPLACE FUNCTION public.get_community_pulse(p_church_id uuid)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  -- Verify the CALLER belongs to the requested church
  IF NOT EXISTS (
    SELECT 1 FROM profiles
    WHERE user_id = v_user_id
      AND church_id = p_church_id
  ) THEN
    RAISE EXCEPTION 'Access denied: caller does not belong to this church';
  END IF;
...
CREATE OR REPLACE FUNCTION public.get_community_pulse_v2(p_church_id uuid)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path TO 'public'
